...
# constraint_bounds = [(-2,2) for i in range(X.shape[1])]
# result =  personalizedIntervention(X, y, model, cardinality = 1, constraint_bounds=constraint_bounds, smoothing_levels = (4,3,2,1))
# results = basePIoptimization(X, model, constraint_bounds=None, l = 0.0))

##### Optimization of personalized intervention #####

def personalizedIntervention(X, model, individual, fixed_features = None,
    ...
    # NOTE: only returns SHIFT, does NOT return individual + optimal_shift! need to do this yourself to find best transformation.
    # model = fitted GP model to training (X, y) pairs.
    # individual = the features of the individual to intervene upon (1-D array)
    ...
    # quantile = which quantile to use (0.05 by default).
    # l = initial maximal regularization penalty to try.
    # eta, max_iter, convergence_thresh = parameters for gradient method.
    # max_l_search: controls binary search: don't try more than this many times, simply chose the largest nonzero terms if we still havent achieved target cardinality.
