...
##### Optimization of populatation UNIFORM intervention #####
...
def sparsePopulationUniform(X, model, cardinality = None,
    ...
    # optimal_uniform = vector of masked entries except for those which should be set to certain values:
...
def smoothedPopulationUniform(X, model, smoothing_levels = (),
...
def populationUniformOptimization(x, model,
                    constraint_bounds=None, l = 0.0, initial_diff = None, 
                    eta = 1.0, max_iter = 1e4, convergence_thresh = 1e-9):
    ...
    if initial_diff is None: # Set initial guess to be uniform intervention where nothing is set.
        ...
        if np.isclose(l, 0):
            initial_diff = feature_means # no masking needed.
        else:
            initial_diff = ma.masked_array(feature_means, mask=[1]*d)
    FIVE_PERCENTILE_ZSCORE = -1.645
    kernel = model.kern

    def populationUniformObj(unif_intervention): # Only returns objective (when unif_intervention is simply numpy array)
        n, d = x.shape
        int_indices = range(d) # features we're setting.
    ...
    def populationSparseUniformObj(unif_intervention): # Only returns objective (when unif_intervention is masked vector)
        n, d = x.shape
    ...
    def populationMaskedUniformObjGrad(unif_intervention): # Returns objective and gradient, which has elements even for the masked dimensions of unif_intervention.
        n, d = x.shape
        int_indices = np.where(np.logical_not(unif_intervention.mask))[0] # features we're setting.
        transformed_pop = x.copy()
        if len(int_indices) > 0:
        ...
        for j in range(n):
            ...
            x_new = transformed_pop[j].reshape(1, d)
        ...
        for i in range(n):
            ...
            x_new_i = transformed_pop[i].reshape(1, d)
            for j in range(n):
                test_point_j = x[j].reshape(1, d)
                x_new_j = transformed_pop[j].reshape(1, d)
                if (i != j):
    ...
    if not jac: # no sparsity in uniform intervention.
        negative_pop_obj = lambda z: -populationUniformObj(z)
        ...
        negative_pop_obj = lambda z: -populationSparseUniformObj(z)
        negative_pop_obj_grad = lambda w: negate_tuple(populationShiftObjGrad(w))
        diff_opt, objective_val = UniformGradDesBacktrack(negative_pop_obj_grad, negative_pop_obj,
...
def UniformGradDesBacktrack(objective_and_grad, objective_nograd, center, guess, l, 
                                 constraint_bounds = None,
...
##### Optimization of populatation SHIFT intervention ######
